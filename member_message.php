    ...
    <div id="member_main">
        ...
        <form action="?action=delete" method="post">
            <table cellspacing="1">
                ...
                ?>
                <tr><td><?php echo $_html['fromuser']?></td><td><a href="member_message_details.php?id=<?php echo $_html['id']?>"><?php echo _title($_html['content'])?></a></td><td><?php echo $_html['date']?></td><td><input name="ids[]" value="<?php echo $_html['id']?>" type="checkbox"/></td></tr>
                <?php
                    }
                    _mysql_free_result($_result);
                ?>
                <tr><td colspan="4"><label for="all">全选<input type="checkbox" name="chkall" id="all"></label><input type="submit" id="delete" value="批量删除" name="delete"></td></tr>
            </table>
        </form>
        <?php
            //调用分页函数
